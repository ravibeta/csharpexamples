...
namespace MatsuoKeywordExtractor
{
    public class Clusterer
    {
        ...
        private int[,] PopulateCooccurrenceMatrix(SortedDictionary<string, int> dict, string[] sentences)
        {

            var topTerms = dict.OrderByDescending(x => x.Value);
            var cutoff = topTerms.Sum(x => x.Value) * 0.3d;
            var sum = 0.0d;
            var frequentTerms = topTerms.TakeWhile(x => { sum += x.Value; return sum < cutoff; }).ToList();
